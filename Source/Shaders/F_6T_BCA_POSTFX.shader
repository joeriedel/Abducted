...
-- Inputs (0 == framebuffer, 1 == colorize texture, 2 == border texture, 3/4/5 == additive)
...
-- add
	...
	local add3 = Node("SampleTexture2D", "add3")
	local addBlend = Node("Add4", "addBlend")
	...
	add3.In.t = MTexture(5)
	add3.In.tc = MTexCoord(5)
	
	addBlend.In.x = add1
	addBlend.In.y = add2
	addBlend.In.z = add3
	addBlend.In.w = colorize
