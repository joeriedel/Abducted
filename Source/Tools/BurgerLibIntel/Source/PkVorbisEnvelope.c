 ...
 ********************************************************************
 ...
 last mod: $Id: PkVorbisEnvelope.c,v 1.4 2003/12/01 16:12:31 burger Exp $
...
void BURGERCALL _ve_envelope_init(envelope_lookup *e,vorbis_info *vi)
{
	...
	e->iir=static_cast<IIRState_t *>(AllocAPointerClear(ch*4*sizeof(*e->iir)));
	e->filtered=static_cast<float **>(AllocAPointerClear(ch*4*sizeof(*e->filtered)));
	e->ch=ch;
	e->storage=128;
	for(i=0;i<ch*4;i+=4){

		IIRStateInit(e->iir+i,cheb_highpass_stages,cheb_highpass10k_gain,
			cheb_highpass10k_A,cheb_highpass_B);
		IIRStateInit(e->iir+i+1,cheb_bandpass_stages,cheb_bandpass6k_gain,
			cheb_bandpass6k_A,cheb_bandpass_B);
		IIRStateInit(e->iir+i+2,cheb_bandpass_stages,cheb_bandpass3k_gain,
			cheb_bandpass3k_A,cheb_bandpass_B);
		IIRStateInit(e->iir+i+3,cheb_bandpass_stages,cheb_bandpass1k_gain,
			cheb_bandpass1k_A,cheb_bandpass_B);

		e->filtered[i]=static_cast<float *>(AllocAPointerClear(e->storage*sizeof(*e->filtered[i])));
		e->filtered[i+1]=static_cast<float *>(AllocAPointerClear(e->storage*sizeof(*e->filtered[i+1])));
		e->filtered[i+2]=static_cast<float *>(AllocAPointerClear(e->storage*sizeof(*e->filtered[i+2])));
		e->filtered[i+3]=static_cast<float *>(AllocAPointerClear(e->storage*sizeof(*e->filtered[i+3])));
	}

}

void BURGERCALL _ve_envelope_clear(envelope_lookup *e){
	int i;
	for(i=0;i<e->ch*4;i++){
		IIRStateDestroy((e->iir+i));
		DeallocAPointer(e->filtered[i]);
	}
	DeallocAPointer(e->filtered);
	DeallocAPointer(e->iir);
	FastMemSet(e,0,sizeof(*e));
}

/* straight threshhold based until we find something that works better
   and isn't patented */
static float BURGERCALL _ve_deltai(envelope_lookup *ve,float *pre,float *post)
{
	long n=ve->winlength;
...
long _ve_envelope_search(vorbis_dsp_state *v)
{
	...
	if(v->pcm_storage>ve->storage){
		...
		for(i=0;i<ve->ch*4;i++) {
			ve->filtered[i]=static_cast<float *>(ResizeAPointer(ve->filtered[i],ve->storage*sizeof(*ve->filtered[i])));
		}
	}
	...
	for(i=0;i<ve->ch;i++){
		...
		IIRState_t *iir0=ve->iir+i*4;
		IIRState_t *iir1=ve->iir+i*4+1;
		IIRState_t *iir2=ve->iir+i*4+2;
		IIRState_t *iir3=ve->iir+i*4+3;
		int flag=1;
		for(j=ve->current;j<v->pcm_current;j++){
			filtered0[j]=IIRStateFilter(iir0,pcm[j]);
			filtered1[j]=IIRStateFilterBand(iir1,pcm[j]);
			filtered2[j]=IIRStateFilterBand(iir2,pcm[j]);
			filtered3[j]=IIRStateFilterBand(iir3,pcm[j]);
			if(pcm[j])flag=0;
		}
...
int BURGERCALL _ve_envelope_mark(vorbis_dsp_state *v)
{
	envelope_lookup *ve=((backend_lookup_state *)(v->backend_state))->ve;
	vorbis_info *vi=v->vi;
	codec_setup_info *ci=static_cast<codec_setup_info *>(vi->codec_setup);
...
void BURGERCALL _ve_envelope_shift(envelope_lookup *e,long shift)
{
	int i;
	for(i=0;i<e->ch*4;i++)
		memmove(e->filtered[i],e->filtered[i]+shift,(e->current-shift)*sizeof(*e->filtered[i]));
	e->current-=shift;
	if(e->prevmark>=0)
		e->prevmark-=shift;
	if(e->mark>=0)
