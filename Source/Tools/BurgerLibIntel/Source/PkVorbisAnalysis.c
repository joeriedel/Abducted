 ...
 ********************************************************************
 ...
 last mod: $Id: PkVorbisAnalysis.c,v 1.4 2003/12/03 08:14:36 burger Exp $
...
int vorbis_analysis(vorbis_block *vb, ogg_packet *op)
{
	...
	/* currently lazy.  Word16 block dispatches to 0, long to 1. */
	...
	if((ret=_mapping_P[type]->forward(vb,b->mode[mode]))!=0) {
		return(ret);
	}
...
#ifdef ANALYSIS
static void _analysis_output_always(char * /* base */,int i,float *v,int n,int bark,int dB)
{
	int j;
	FILE *of;
	char buffer[80];
...
void BURGERCALL _analysis_output(char * /* base */,int /* i */,float * /* v */,int /* n */,int /* bark */,int /* dB */)
{
#ifdef ANALYSIS
	if (analysis_noisy) {
