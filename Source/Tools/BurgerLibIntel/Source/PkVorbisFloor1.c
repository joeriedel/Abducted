 ...
 ********************************************************************
 ...
 last mod: $Id: PkVorbisFloor1.c,v 1.6 2003/12/03 08:14:36 burger Exp $
...
static vorbis_info_floor *floor1_copy_info (vorbis_info_floor *i)
{
	...
	vorbis_info_floor1 *ret=static_cast<vorbis_info_floor1 *>(AllocAPointer(sizeof(*ret)));
	FastMemCpy(ret,info,sizeof(*ret));
	return (vorbis_info_floor *)(ret);
}
...
static vorbis_info_floor *floor1_unpack (vorbis_info *vi,oggpack_buffer *opb){
  ...
  vorbis_info_floor1 *info=static_cast<vorbis_info_floor1 *>(AllocAPointerClear(sizeof(*info)));
  /* read partitions */
 ...
 err_out:
  floor1_free_info((vorbis_info_floor *)info);
  return(NULL);
}

static int ANSICALL icomp(const void *a,const void *b){
  return(**(int **)a-**(int **)b);
}
...
static vorbis_look_floor *floor1_look(vorbis_dsp_state * /* vd */,vorbis_info_mode * /*mi*/,
  ...
  vorbis_info_floor1 *info=(vorbis_info_floor1 *)input;
...
static int fit_line(lsfit_acc *a,int fits,int *y0,int *y1){
  ...
  
  {
    ...
    *y0=(int)floor((a+b*x0)+0.5f);
    *y1=(int)floor((a+b*x1)+0.5f);
...
static int floor1_forward(vorbis_block *vb,vorbis_look_floor *input,
			  float *mdct, const float *logmdct,   /* input */
			  const float *logmask, const float *logmax, /* input */
			  float *codedflr){          /* output */
  static int seq=0;
  long i,j,k,l;
  ...
  FastMemSet(fit_flag,0,sizeof(fit_flag));
  ...
  if(nonzero){
	...
	int vorbclass=info->partitionclass[i];
	int cdim=info->class_dim[vorbclass];
	int csubbits=info->class_subs[vorbclass];
	int csub=1<<csubbits;
	...
	if(csubbits){
	  int maxval[8];
	  for(k=0;k<csub;k++){
	    int booknum=info->class_subbook[vorbclass][k];
	    if(booknum<0){
	      maxval[k]=1;
	    }else{
	      maxval[k]=sbooks[info->class_subbook[vorbclass][k]]->entries;
	    }
	  }
	  for(k=0;k<cdim;k++){
	    for(l=0;l<csub;l++){
	  ...
	  /* write it */
	  look->phrasebits+=
	  vorbis_book_encode(books+info->class_book[vorbclass],cval,&vb->opb);
	...
	/* write post values */
	for(k=0;k<cdim;k++){
	  int book=info->class_subbook[vorbclass][bookas[k]];
	  if(book>=0){
...
#ifdef TRAIN_FLOOR1
	    {
	      ...
	      sprintf(buffer,"line_%dx%ld_%dsub%d.vqd",
		      vb->pcmend/2,posts-2,vorbclass,bookas[k]);
	      of=fopen(buffer,"a");
	      fprintf(of,"%d\n",fit_valueB[j+k]);
	      fclose(of);
	    }
    ...
    FastMemSet(codedflr,0,n*sizeof(*codedflr));
    FastMemSet(mdct,0,n*sizeof(*mdct));
  }
  seq++;
  return(nonzero);
}
...
static void *floor1_inverse1(vorbis_block *vb,vorbis_look_floor *input){
  ...
  if(oggpack_read1(&vb->opb)==1){
    ...
    for(i=0,j=2;i<info->partitions;i++){
      ...
      if(csubbits){
	cval=vorbis_book_decode(books+info->class_book[vorbclass],&vb->opb);

	if(cval==-1)goto eop;
      }

      for(k=0;k<cdim;k++){
	int book=info->class_subbook[vorbclass][cval&(csub-1)];
	cval>>=csubbits;
	if(book>=0){
...
static int floor1_inverse2(vorbis_block *vb,vorbis_look_floor *input,void *memo,
			  float *output){
  vorbis_look_floor *look=(vorbis_look_floor *)input;
  vorbis_info_floor1 *info=look->vi;
  ...
  if(memo){
	...
	render_line(lx,hx,ly,hy,output);
	
	lx=hx;
	ly=hy;
      }
    }
    for(j=hx;j<n;j++)output[j]*=ly; /* be certain */    
    return(1);
  }
  FastMemSet(output,0,sizeof(*output)*n);
  return(0);
}
