...
static void transdecode_master_selection (CJPegDecompress * cinfo)
{
...
struct jvirt_barray_control * * BURGERCALL jpeg_read_coefficients (CJPegDecompress * cinfo)
{
	if (cinfo->m_GlobalState == JPeg70::DSTATE_READY) {
		/* First call: initialize active modules */
		transdecode_master_selection(cinfo);
		cinfo->m_GlobalState = JPeg70::DSTATE_RDCOEFS;
	}
	if (cinfo->m_GlobalState == JPeg70::DSTATE_RDCOEFS) {
	...
	*/
	if ((cinfo->m_GlobalState == JPeg70::DSTATE_STOPPING ||
		cinfo->m_GlobalState == JPeg70::DSTATE_BUFIMAGE) && cinfo->buffered_image) {
