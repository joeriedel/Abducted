 ...
 ********************************************************************
  ...
  last mod: $Id: PkVorbislsp.c,v 1.3 2003/11/22 05:12:08 burger Exp $
...
/* catch this in the build system; we include for
	compilers (like gcc) that can't inline across modules */

/* side effect: changes *lsp to cosines of lsp */
void BURGERCALL vorbis_lsp_to_curve(float *curve,int *map,int n,int ln,float *lsp,int m,
...
void BURGERCALL vorbis_lsp_to_curve(float *curve,int *map,int n,int ln,float *lsp,int m,
  ...
  for(i=0;i<m;i++)ilsp[i]=vorbis_coslook_i(lsp[i]/PK_PI*65536.f+.5f);
	...
	float amp,float ampoffset)
{
	...
	while(i<n){
		...
		q=fromdB(amp/(float)sqrt(p+q)-ampoffset);
...
static void BURGERCALL cheby(float *g, int ord) {
  int i, j;
...
static int ANSICALL comp(const void *a,const void *b){
  if(*(float *)a<*(float *)b)
    return(1);
  else
    return(-1);
...
static int BURGERCALL Laguerre_With_Deflation(float *a,int ord,float *r){
  int i,m;
  ...
  for(m=ord;m>0;m--){
	...
	if(denom>-(EPSILON))denom=-(EPSILON);
      ...
      if(FloatAbs(delta/newval)<10e-12)break; 
      lastdelta=delta;
    }
...
static int BURGERCALL Newton_Raphson(float *a,int ord,float *r)
{
	int i, k, count=0;
	double error=1.f;
...
int BURGERCALL vorbis_lpc_to_lsp(float *lpc,float *lsp,int m)
{
	int order2=(m+1)>>1;
