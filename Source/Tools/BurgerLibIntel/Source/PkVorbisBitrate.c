 ...
 ********************************************************************
 ...
 last mod: $Id: PkVorbisBitrate.c,v 1.5 2003/12/01 16:12:30 burger Exp $
...
static long BURGERCALL BINBITS(bitrate_manager_state *bm,long pos,long inbin){
...
static long BURGERCALL LACING_ADJUST(long bits){
  int addto=((bits+7)/8+1)/256+1;
  return( ((bits+7)/8+addto)*8 );
}

static double BURGERCALL floater_interpolate(bitrate_manager_state *bm,vorbis_info *vi,
				  double desired_rate){
  int bin=(int)(bm->avgfloat*BITTRACK_DIVISOR-1.);
  double lobitrate;
  double hibitrate;
...
/* try out a new limit */
static long BURGERCALL limit_sum(bitrate_manager_state *bm,int limit){
...
void BURGERCALL vorbis_bitrate_init(vorbis_info *vi,bitrate_manager_state *bm){
  ...
  if(bi){
	...
	oggpack_writeinit(bm->queue_packet_buffers+i);
      ...
      bm->queue_packet_buffers=static_cast<oggpack_buffer *>(AllocAPointerClear(sizeof(*bm->queue_packet_buffers)));
      bm->queue_packets=static_cast<ogg_packet *>(AllocAPointerClear(sizeof(*bm->queue_packets)));
      oggpack_writeinit(bm->queue_packet_buffers);
    }      
  }
}

void BURGERCALL vorbis_bitrate_clear(bitrate_manager_state *bm)
{
	int i;
	if(bm){
		DeallocAPointer(bm->queue_binned);
		DeallocAPointer(bm->queue_actual);
		DeallocAPointer(bm->avg_binacc);
		DeallocAPointer(bm->minmax_binstack);
		DeallocAPointer(bm->minmax_posstack);
		DeallocAPointer(bm->minmax_limitstack);
		if(bm->queue_packet_buffers){
			...
			DeallocAPointer(bm->queue_packet_buffers);
		}
		DeallocAPointer(bm->queue_packets);
		FastMemSet(bm,0,sizeof(*bm));
	}
}

int BURGERCALL vorbis_bitrate_managed(vorbis_block *vb)
{
...
int BURGERCALL vorbis_bitrate_maxmarkers(void)
{
	return 8*BITTRACK_DIVISOR;
}

/* finish taking in the block we just processed */
int BURGERCALL vorbis_bitrate_addblock(vorbis_block *vb){
  int i; 
  ...
  if(!bm->queue_binned){
    ...
    FastMemCpy(&temp,bm->queue_packet_buffers,sizeof(vb->opb));
    FastMemCpy(bm->queue_packet_buffers,&vb->opb,sizeof(vb->opb));
    FastMemCpy(&vb->opb,&temp,sizeof(vb->opb));
  ...
  /* swap packet buffers */
  FastMemCpy(&temp,bm->queue_packet_buffers+head,sizeof(vb->opb));
  FastMemCpy(bm->queue_packet_buffers+head,&vb->opb,sizeof(vb->opb));
  FastMemCpy(&vb->opb,&temp,sizeof(vb->opb));

  /* save markers */
  FastMemCpy(head_ptr,vbi->packet_markers,sizeof(*head_ptr)*bins);
  ...
  if(bm->avg_binacc){
	...
	double newval=bi->avgfloat_initial,slew;
	int bin;
	
	if(upper>0. && upper<newval)newval=upper;
	if(lower<bi->avgfloat_minimum)
	  lower=bi->avgfloat_minimum;
	if(lower>newval)newval=lower;
	
	slew=(newval-bm->avgfloat)/samples*vi->rate;
	
	if(slew<bi->avgfloat_downslew_max)
	  newval=bm->avgfloat+bi->avgfloat_downslew_max/vi->rate*samples;
	if(slew>bi->avgfloat_upslew_max)
	  newval=bm->avgfloat+bi->avgfloat_upslew_max/vi->rate*samples;
	
	bm->avgfloat=newval;

	/* apply the average floater to new blocks */
	bin=(int)(bm->avgfloat*(BITTRACK_DIVISOR<<BITTRACK_BPT));
    ...
    /* if we're not using an average tracker, the 'float' is nailed to
       ...
       to deal properly */
    long bin=(long)(bi->avgfloat_initial*(BITTRACK_DIVISOR<<BITTRACK_BPT));
  ...
  if(bm->minmax_binstack){
	...
	*/
	...
	FastMemSet(&bm->minmax_binstack[stackctr*bins*2],
	       0,
...
int BURGERCALL vorbis_bitrate_flushpacket(vorbis_dsp_state *vd,ogg_packet *op)
{
	backend_lookup_state  *b=static_cast<backend_lookup_state *>(vd->backend_state);
	bitrate_manager_state *bm=&b->bms;
	...
	if(bm->queue_size==0){
		...
		FastMemCpy(op,bm->queue_packets,sizeof(*op));
		bm->queue_head=0;
		...
		FastMemCpy(op,bm->queue_packets+bm->next_to_flush,sizeof(*op));
