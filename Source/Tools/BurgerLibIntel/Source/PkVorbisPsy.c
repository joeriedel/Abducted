 ...
 ********************************************************************
 ...
 last mod: $Id: PkVorbisPsy.c,v 1.5 2003/12/01 16:12:31 burger Exp $
...
vorbis_look_psy_global *BURGERCALL _vp_global_look(vorbis_info *vi)
{
	codec_setup_info *ci=static_cast<codec_setup_info *>(vi->codec_setup);
	vorbis_info_psy_global *gi=&ci->psy_g_param;
	vorbis_look_psy_global *look=static_cast<vorbis_look_psy_global *>(AllocAPointerClear(sizeof(*look)));
...
void BURGERCALL _vp_global_free(vorbis_look_psy_global *look)
{
	if(look){
...
#if 0
static void BURGERCALL _vi_gpsy_free(vorbis_info_psy_global *i)
{
	if(i){
		FastMemSet(i,0,sizeof(*i));
		DeallocAPointer(i);
	}
}
#endif

void BURGERCALL _vi_psy_free(vorbis_info_psy *i)
{
	if(i){
		FastMemSet(i,0,sizeof(*i));
		DeallocAPointer(i);
...
vorbis_info_psy *BURGERCALL _vi_psy_copy(vorbis_info_psy *i)
{
	vorbis_info_psy *ret=static_cast<vorbis_info_psy *>(AllocAPointer(sizeof(*ret)));
	FastMemCpy(ret,i,sizeof(*ret));
	return(ret);
}
...
/* ATH is the only bit left on a Bark scale.  No reason to change it
   right now */
static void BURGERCALL set_curve(float *ref,float *c,int n, float crate)
{
	int i,j=0;

	for(i=0;i<MAX_BARK-1;i++){
		int endpos=(int)floor((fromBARK((float)(i+1))*2*n/crate)+0.5f);
		float base=ref[i];
		if(j<endpos){
...
static void BURGERCALL min_curve(float *c,float *c2)
{
	int i;  
...
static void BURGERCALL max_curve(float *c,float *c2)
{
	int i;  
	for(i=0;i<EHMER_MAX;i++)
		if(c2[i]>c[i])
...
static void BURGERCALL attenuate_curve(float *c,float att){
  int i;
  for(i=0;i<EHMER_MAX;i++)
    c[i]+=att;
}

static void BURGERCALL interp_curve(float *c,float *c1,float *c2,float del){
  int i;
  for(i=0;i<EHMER_MAX;i++)
    c[i]=c2[i]*del+c1[i]*(1.f-del);
}

static void BURGERCALL setup_curve(float **c,
			int band,
			float *curveatt_dB){
  int i,j;
  float ath[EHMER_MAX];
  ...
  FastMemCpy(c[0]+2,c[4]+2,sizeof(*c[0])*EHMER_MAX);
  FastMemCpy(c[2]+2,c[4]+2,sizeof(*c[2])*EHMER_MAX);
  ...
  for(i=0;i<EHMER_MAX;i++){
    float oc_min=band*.5f+(i-EHMER_OFFSET)*.125f;
    float oc_max=band*.5f+(i-EHMER_OFFSET+1)*.125f;
    float bark=toBARK(fromOC(oc_min));
    int ibark=(int)floor(bark);
  ...
  for(i=0;i<P_LEVELS;i++){
    ...
    FastMemCpy(tempc[i],ath,EHMER_MAX*sizeof(*tempc[i]));
...
void BURGERCALL _vp_psy_init(vorbis_look_psy *p,vorbis_info_psy *vi,
  ...
  p->shiftoc=(long)floor((log(gi->eighth_octave_lines*8.f)/log(2.f))+0.5f)-1;

  p->firstoc=(long)(toOC(.25f*rate/n)*(1<<(p->shiftoc+1))-gi->eighth_octave_lines);
  maxoc=(long)(toOC((n*.5f-.25f)*rate/n)*(1<<(p->shiftoc+1))+.5f);
  p->total_octave_lines=maxoc-p->firstoc+1;

  if(vi->ath)
    p->ath=static_cast<float *>(AllocAPointer(n*sizeof(*p->ath)));
  p->octave=static_cast<SWord32 *>(AllocAPointer(n*sizeof(*p->octave)));
  p->bark=static_cast<SWord32 *>(AllocAPointer(n*sizeof(*p->bark)));
  p->vi=vi;
  p->n=n;
  p->rate=rate;
  ...
  for(i=0;i<n;i++)
    p->octave[i]=(long)(toOC((i*.5f+.25f)*rate/n)*(1<<(p->shiftoc+1))+.5f);

  p->tonecurves=static_cast<float ***>(AllocAPointer(P_BANDS*sizeof(*p->tonecurves)));
  p->noisethresh=static_cast<float *>(AllocAPointer(n*sizeof(*p->noisethresh)));
  p->noiseoffset=static_cast<float *>(AllocAPointer(n*sizeof(*p->noiseoffset)));
  for(i=0;i<P_BANDS;i++)
    p->tonecurves[i]=static_cast<float **>(AllocAPointer(P_LEVELS*sizeof(*p->tonecurves[i])));
  
  for(i=0;i<P_BANDS;i++)
    for(j=0;j<P_LEVELS;j++)
      p->tonecurves[i][j]=static_cast<float *>(AllocAPointer((EHMER_MAX+2)*sizeof(*p->tonecurves[i][j])));
  

  /* OK, yeah, this was a silly way to do it */
  FastMemCpy(p->tonecurves[0][4]+2,tone_125_40dB_SL,sizeof(*p->tonecurves[0][4])*EHMER_MAX);
  FastMemCpy(p->tonecurves[0][6]+2,tone_125_60dB_SL,sizeof(*p->tonecurves[0][6])*EHMER_MAX);
  FastMemCpy(p->tonecurves[0][8]+2,tone_125_80dB_SL,sizeof(*p->tonecurves[0][8])*EHMER_MAX);
  FastMemCpy(p->tonecurves[0][10]+2,tone_125_100dB_SL,sizeof(*p->tonecurves[0][10])*EHMER_MAX);

  FastMemCpy(p->tonecurves[2][4]+2,tone_125_40dB_SL,sizeof(*p->tonecurves[2][4])*EHMER_MAX);
  FastMemCpy(p->tonecurves[2][6]+2,tone_125_60dB_SL,sizeof(*p->tonecurves[2][6])*EHMER_MAX);
  FastMemCpy(p->tonecurves[2][8]+2,tone_125_80dB_SL,sizeof(*p->tonecurves[2][8])*EHMER_MAX);
  FastMemCpy(p->tonecurves[2][10]+2,tone_125_100dB_SL,sizeof(*p->tonecurves[2][10])*EHMER_MAX);

  FastMemCpy(p->tonecurves[4][4]+2,tone_250_40dB_SL,sizeof(*p->tonecurves[4][4])*EHMER_MAX);
  FastMemCpy(p->tonecurves[4][6]+2,tone_250_60dB_SL,sizeof(*p->tonecurves[4][6])*EHMER_MAX);
  FastMemCpy(p->tonecurves[4][8]+2,tone_250_80dB_SL,sizeof(*p->tonecurves[4][8])*EHMER_MAX);
  FastMemCpy(p->tonecurves[4][10]+2,tone_250_100dB_SL,sizeof(*p->tonecurves[4][10])*EHMER_MAX);

  FastMemCpy(p->tonecurves[6][4]+2,tone_500_40dB_SL,sizeof(*p->tonecurves[6][4])*EHMER_MAX);
  FastMemCpy(p->tonecurves[6][6]+2,tone_500_60dB_SL,sizeof(*p->tonecurves[6][6])*EHMER_MAX);
  FastMemCpy(p->tonecurves[6][8]+2,tone_500_80dB_SL,sizeof(*p->tonecurves[6][8])*EHMER_MAX);
  FastMemCpy(p->tonecurves[6][10]+2,tone_500_100dB_SL,sizeof(*p->tonecurves[6][10])*EHMER_MAX);

  FastMemCpy(p->tonecurves[8][4]+2,tone_1000_40dB_SL,sizeof(*p->tonecurves[8][4])*EHMER_MAX);
  FastMemCpy(p->tonecurves[8][6]+2,tone_1000_60dB_SL,sizeof(*p->tonecurves[8][6])*EHMER_MAX);
  FastMemCpy(p->tonecurves[8][8]+2,tone_1000_80dB_SL,sizeof(*p->tonecurves[8][8])*EHMER_MAX);
  FastMemCpy(p->tonecurves[8][10]+2,tone_1000_100dB_SL,sizeof(*p->tonecurves[8][10])*EHMER_MAX);

  FastMemCpy(p->tonecurves[10][4]+2,tone_2000_40dB_SL,sizeof(*p->tonecurves[10][4])*EHMER_MAX);
  FastMemCpy(p->tonecurves[10][6]+2,tone_2000_60dB_SL,sizeof(*p->tonecurves[10][6])*EHMER_MAX);
  FastMemCpy(p->tonecurves[10][8]+2,tone_2000_80dB_SL,sizeof(*p->tonecurves[10][8])*EHMER_MAX);
  FastMemCpy(p->tonecurves[10][10]+2,tone_2000_100dB_SL,sizeof(*p->tonecurves[10][10])*EHMER_MAX);

  FastMemCpy(p->tonecurves[12][4]+2,tone_4000_40dB_SL,sizeof(*p->tonecurves[12][4])*EHMER_MAX);
  FastMemCpy(p->tonecurves[12][6]+2,tone_4000_60dB_SL,sizeof(*p->tonecurves[12][6])*EHMER_MAX);
  FastMemCpy(p->tonecurves[12][8]+2,tone_4000_80dB_SL,sizeof(*p->tonecurves[12][8])*EHMER_MAX);
  FastMemCpy(p->tonecurves[12][10]+2,tone_4000_100dB_SL,sizeof(*p->tonecurves[12][10])*EHMER_MAX);

  FastMemCpy(p->tonecurves[14][4]+2,tone_8000_40dB_SL,sizeof(*p->tonecurves[14][4])*EHMER_MAX);
  FastMemCpy(p->tonecurves[14][6]+2,tone_8000_60dB_SL,sizeof(*p->tonecurves[14][6])*EHMER_MAX);
  FastMemCpy(p->tonecurves[14][8]+2,tone_8000_80dB_SL,sizeof(*p->tonecurves[14][8])*EHMER_MAX);
  FastMemCpy(p->tonecurves[14][10]+2,tone_8000_100dB_SL,sizeof(*p->tonecurves[14][10])*EHMER_MAX);

  FastMemCpy(p->tonecurves[16][4]+2,tone_16000_40dB_SL,sizeof(*p->tonecurves[16][4])*EHMER_MAX);
  FastMemCpy(p->tonecurves[16][6]+2,tone_16000_60dB_SL,sizeof(*p->tonecurves[16][6])*EHMER_MAX);
  FastMemCpy(p->tonecurves[16][8]+2,tone_16000_80dB_SL,sizeof(*p->tonecurves[16][8])*EHMER_MAX);
  FastMemCpy(p->tonecurves[16][10]+2,tone_16000_100dB_SL,sizeof(*p->tonecurves[16][10])*EHMER_MAX);
  ...
  for(i=1;i<P_BANDS;i+=2)
    for(j=4;j<P_LEVELS;j+=2){
      FastMemCpy(p->tonecurves[i][j]+2,p->tonecurves[i-1][j]+2,EHMER_MAX*sizeof(*p->tonecurves[i][j]));
  ...
  for(i=0;i<n;i++){
    float halfoc=toOC((i+.5f)*rate/(2.f*n))*2.0f;
    int inthalfoc;
    float del;
...
void BURGERCALL _vp_psy_clear(vorbis_look_psy *p){
  int i,j;
  if(p){
    if(p->ath)DeallocAPointer(p->ath);
    if(p->octave)DeallocAPointer(p->octave);
    if(p->bark)DeallocAPointer(p->bark);
    if(p->tonecurves){
      for(i=0;i<P_BANDS;i++){
	for(j=0;j<P_LEVELS;j++){
	  DeallocAPointer(p->tonecurves[i][j]);
	}
	DeallocAPointer(p->tonecurves[i]);
      }
      DeallocAPointer(p->tonecurves);
    }
    DeallocAPointer(p->noiseoffset);
    DeallocAPointer(p->noisethresh);
    FastMemSet(p,0,sizeof(*p));
  }
}

/* octave/(8*eighth_octave_lines) x scale and dB y scale */
static void BURGERCALL seed_curve(float *seed,const float **curves,float amp,
	int oc, int n,int linesper,float dBoffset)
{
	...
	curve=posts+2;
	post1=(int)posts[1];
	seedptr=(int)(oc+(posts[0]-16)*linesper-(linesper>>1));
...
static void BURGERCALL seed_loop(vorbis_look_psy *p,
		      const float ***curves,
		      const float *f, 
		      const float *flr,
		      float *seed,
...
static void BURGERCALL seed_chase(float *seeds, int linesper, long n)
{
	long  *posstack=static_cast<long *>(AllocAPointer(n*sizeof(*posstack)));
	float *ampstack=static_cast<float *>(AllocAPointer(n*sizeof(*ampstack)));
	long   stack=0;
	long   pos=0;
	long   i;
	...
	DeallocAPointer(posstack);
	DeallocAPointer(ampstack);

}

/* bleaugh, this is more complicated than it needs to be */
static void BURGERCALL max_seeds(vorbis_look_psy *p,
		      float *seed,
		      float *flr){
...
static void BURGERCALL bark_noise_hybridmp(int n,const long *b,
				const float *f,
				float *noise,
				const float offset,
				const int fixed){
  ...
  for(i=0;i<n;i++){
    ...
    {    
      ...
      noise[i]=(float)(va-offset);
    }
  }
}

   
void BURGERCALL _vp_remove_floor(vorbis_look_psy *p,
		      float *mdct,
		      float *codedflr,
		      float *residue){ 
  int i,n=p->n;
...
void BURGERCALL _vp_compute_mask(vorbis_look_psy *p,
		      float *logfft, 
		      float *logmdct, 
		      float *logmask, 
  ...
  float *seed=static_cast<float *>(AllocAPointer(sizeof(*seed)*p->total_octave_lines));
  for(i=0;i<p->total_octave_lines;i++)seed[i]=NEGINF;

  /* noise masking */
	if(p->vi->noisemaskp){
		float *work=static_cast<float *>(AllocAPointer(n*sizeof(*work)));
  ...
  if(i==n)
    for(i=0;i<n;i++)logmask[i]=NEGINF;
	  ...
	  for(i=0;i<n;i++) {
		  if (logfft[i]<logmdct[i]) {
			  logfft[i]=logmdct[i];
		  }
	  }
	}
	seq++;
	DeallocAPointer(seed);
}

float BURGERCALL _vp_ampmax_decay(float amp,vorbis_dsp_state *vd){
...
static void BURGERCALL couple_lossless(float A, float B, 
			    float granule,float igranule,
			    float *mag, float *ang,
			    int flip_p){

  if(FloatAbs(A)>FloatAbs(B)){
    A=(float)floor((A*igranule)+0.5f)*granule; /* must be done *after* the comparison */
    B=(float)floor((B*igranule)+0.5f)*granule;
  
    *mag=A; *ang=(A>0.f?A-B:B-A);
  }else{
    A=(float)floor((A*igranule)+0.5f)*granule;
    B=(float)floor((B*igranule)+0.5f)*granule;
  
    *mag=B; *ang=(B>0.f?A-B:B-A);
  }

  if(flip_p && *ang>FloatAbs(*mag)*1.9999f){
    *ang= -FloatAbs(*mag)*2.f;
    *mag= -*mag;
  }
}

static void BURGERCALL couple_point(float A, float B, float fA, float fB, 
			 float granule,float igranule,
			 float fmag, float *mag, float *ang){

  float origmag=(float)FAST_HYPOT(A*fA,B*fB),corr;
  ...
  if(fmag!=0.f){
    ...
    corr=origmag/(float)FAST_HYPOT(fmag*fA,fmag*fB);
    *mag=(float)floor((*mag*corr*igranule)+0.5f)*granule; 
    *ang=0.f;
...
void BURGERCALL _vp_quantize_couple(vorbis_look_psy *p,
